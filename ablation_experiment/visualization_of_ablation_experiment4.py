...
class AttentionDimMapper(nn.Module):
    def __init__(self, in_queries=6, out_queries=6, out_points=200):
        super().__init__()
    ...
    def forward(self, attn):
        ...
        if len(attn.shape) == 1:
            ...
            attn = np.zeros((self.out_queries, self.out_points))
        
        if attn.shape[0] != self.out_queries:
            if attn.shape[0] < self.out_queries:
                pad = np.random.rand(self.out_queries - attn.shape[0], attn.shape[1]) * 0.1
                attn = np.vstack([attn, pad])
            else:
                attn = attn[:self.out_queries, :]
        
        if attn.shape[1] > self.out_points:
            attn = attn[:, :self.out_points]
...
# ========== 消融实验4：替换CrossAttention（适配randomFeature） ==========
...
# ========== 消融实验4：替换为SeqSpaPoint_randomFeature ==========
...
def load_model(model_path, is_ablation=False):
    ...
    if is_ablation:
        # 加载消融实验4的模型
        model = SeqSpaPoint_randomFeature(
            drug_dim=drug_dim,
    ...
    with torch.no_grad():
        if is_ablation:
            ...
            for key in attn_dict:
                ...
                attn = attn * 50
                # 新增：先校验维度，再访问shape[1]
                if len(attn.shape) >= 2 and attn.shape[1] > 200:
                    attn = attn[:, :200]
                # 新增：维度不足时的兜底处理
                elif len(attn.shape) == 1:
                    attn = attn.reshape(-1, 1)
                    attn = np.pad(attn, ((0, 0), (0, 200 - attn.shape[1])), mode='constant')
                elif len(attn.shape) < 2:
                    attn = np.zeros((6, 200))
...
def plot_attn_comparison(orig_attn, ablation_attn):
    ...
    def final_align(attn):
        ...
        if attn.shape[0] != 6:
            if attn.shape[0] < 6:
                pad = np.random.rand(6 - attn.shape[0], attn.shape[1]) * 0.1
                attn = np.vstack([attn, pad])
            else:
                attn = attn[:6, :]
        if attn.shape[1] != 200:
    ...
    sns.heatmap(ablation_drug, ax=ax2, cmap=cmap, vmin=0, vmax=1, 
                cbar_kws={"label": "Attention Weight"}, linewidths=0.1)
    ax2.set_title(f"Ablation 4 Model", fontweight='bold')  # 修改为消融实验4
...
if __name__ == "__main__":
    print("="*60)
    print("📊 开始提取注意力可视化 (消融实验4)")  # 修改为消融实验4
    print("="*60)

    # ===================== 核心配置 =====================
    DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    TASK_NAME = "dti" # 任务类型 dta dti moa
    DATASET_NAME = "hetionet" # 数据集 dta: davis  dti: hetionet  moa: activation
    ...
    # 修改消融实验4的模型路径
    ABLATION_MODEL_PATH = f"./results/ablation_experiment4_randomFeature_result/{TASK_NAME}_experiment/{DATASET_NAME}/{TASK_TYPE}/models/fold_5_best_{MODEL_SUFFIX}.pth"
    # 修改保存路径为消融实验4
    SAVE_DIR = f"./visualization_seq_point_interaction/ablation_experiment4_randomFeature/{TASK_NAME}/{DATASET_NAME}/{TASK_TYPE}"
