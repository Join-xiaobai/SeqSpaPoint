...
# 替换为消融实验3的模型（SeqSpaPoint_nofeature）
...
def plot_attn_comparison(orig_attn, ablation_attn):
    ...
    sns.heatmap(ablation_drug, ax=ax2, cmap=cmap, vmin=0, vmax=1, 
                cbar_kws={"label": "Attention Weight"}, linewidths=0.1)
    ax2.set_title(f"Ablation 3 Model", fontweight='bold')  # 修改为消融实验3
...
if __name__ == "__main__":
    print("="*60)
    print("📊 开始提取注意力可视化 (消融实验3)")  # 修改为消融实验3
    print("="*60)
    ...
    # 修改消融实验3的模型路径
    ABLATION_MODEL_PATH = f"./results/ablation_experiment3_nofeature_result/{TASK_NAME}_experiment/{DATASET_NAME}/{TASK_TYPE}/models/fold_5_best_{MODEL_SUFFIX}.pth"
    # 修改保存路径为消融实验3
    SAVE_DIR = f"./visualization_seq_point_interaction/ablation_experiment3_nofeature/{TASK_NAME}/{DATASET_NAME}/{TASK_TYPE}"
