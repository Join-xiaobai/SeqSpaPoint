...
def load_model(model_path, is_ablation=False):
    ...
    if is_ablation:
        ...
        model.attn_mapper = AttentionDimMapper(in_queries=6, out_queries=6, out_points=200).to(DEVICE)
    else:
...
def plot_attn_comparison(orig_attn, ablation_attn):
    ...
    sns.heatmap(ablation_drug, ax=ax2, cmap=cmap, vmin=0, vmax=1, 
                cbar_kws={"label": "Attention Weight"}, linewidths=0.1)
    ax2.set_title(f"Ablation 2 Model", fontweight='bold')
...
if __name__ == "__main__":
    ...
    SAVE_DIR = f"./visualization_seq_point_interaction/ablation_experiment2_nopoint/{TASK_NAME}/{DATASET_NAME}/{TASK_TYPE}"
