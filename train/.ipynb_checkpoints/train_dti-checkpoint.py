...
def focal_binary_loss(
    ...
    """
    DTI专用Focal Loss（解决类别不平衡）
    :param y_pred: 模型输出logits (B,)
    :param y_true: 真实标签 (B,)
    :param gamma: 聚焦参数
    :param alpha: 类别权重参数
    :param use_numerical_stability: 数值稳定保护
    :return: 标量损失值
    """
    ...
    if use_numerical_stability:
        y_pred = torch.clamp(y_pred, -10, 10)  # 防止sigmoid溢出
...
def train_with_cv(
    ...
    dropout_rate=0.15,
    # ========== DTI可控优化参数 ==========
    use_focal_loss=True,          # 是否使用Focal Loss（替代普通BCE）
    use_class_weight=True,        # 是否使用类别加权采样
    use_numerical_stability=True, # 数值稳定保护
    ...
    if task_type != "dti":
        raise ValueError(f"仅支持 task_type='dti'，但收到: {task_type}")
    ...
    global_log_path = os.path.join(BASE_SAVE_DIR, "dti_global_train_log.txt")
    sys.stdout = Logger(global_log_path)

    # 打印实验信息
    print(f"===== DTI实验开始时间：{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} =====")
    print(f"  - 计算设备：{device}")
    ...
    print(f"  - 损失函数：{'Focal BCEWithLogits' if use_focal_loss else '普通BCEWithLogits'} (γ={focal_gamma}, α={focal_alpha})")
    print(f"  - 学习率调度：CosineAnnealing with Warmup")
    print(f"  - 模型：num_queries=6, dropout={dropout_rate}")
    print(f"  - 核心逻辑：K折 + 训练集 + 测试集做评估")
    print()

    # 遍历不同分割场景（warm/drug_cold/target_cold）
    for split_type in split_types:
        print("=" * 80)
        print(f"开始 {split_type.upper()} 场景DTI训练（K={num_folds}折）")
        print("=" * 80)
        ...
        try:
            ...
            print(f"✅ DTI数据集加载成功 | Drug 嵌入维度: {drug_dim} | Target 嵌入维度: {target_dim}")
        ...
        for fold_idx, (_, _) in enumerate(kf.split(dataset.train_indices)):
            # 关键修改：K折划分出的验证集合并入训练集（直接用全局训练集作为该折的训练集）
            fold_train_indices = dataset.train_indices  # 无验证集，全部用于训练
            print("-" * 80)
            ...
            for epoch in range(num_epochs):
                ...
                current_test_auprc = test_metrics['auprc']  # 按测试集AUPRC评估
        ...
        print("=" * 80)
        print(f"{split_type.upper()} 场景{num_folds}折交叉验证结果汇总（测试集）")
        for name, (mean, std) in metrics_summary.items():
            print(f"  {name}: {mean} ± {std}")
        print("=" * 80 + "\n")

    # 实验结束
    print(f"===== DTI实验结束时间：{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} =====")
    print(f"所有DTI实验完成！结果保存至：{BASE_SAVE_DIR}")
    sys.stdout.close()
    sys.stdout = sys.__stdout__